...
function bindEvents(block) {
  ...
  // console.log("slideIndicators: ", slideIndicators)
...
export default async function decorate(block) {
  carouselId += 1;
  const isJSONCarousel = block.classList.contains('json');
  ...
  if (isJSONCarousel) {
    ...
    cardData.forEach((card, idx) => {
      ...
      if (isImageCards) {
        ...
        createdSlide.innerHTML = `
        <a href="${card.link}" aria-label="${card.name}" title="${card.name}" class="">
          <div class="cards-card-image">
            ${picture.outerHTML}
          </div>
          <div class="cards-card-body">
            <p>${card.name}</p>
            <div class="prices">
              <span>$${card['sale-price']}</span>
              <span>$${card['retail-price']}</span>
              <span>${card.discount}% OFF</span>
            </div>
          </div>
        </a>
      `;
