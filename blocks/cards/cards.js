    ...
    [...block.children].forEach((row) => {
      ...
      [...li.children].forEach((div) => {
        if (div.children.length === 1 && div.querySelector('picture')) {
          ...
          //remove hardcoded value from doc-based
          const hardcodedValue = div.querySelector('p:last-of-type')
          const value = hardcodedValue.textContent;
          li.classList.add(`${value}`);
          hardcodedValue.remove();

          //relocate title
          const title = div.querySelector('p:first-of-type');
          const newDiv = document.createElement('div');
          newDiv.classList.add('cards-card-title');
