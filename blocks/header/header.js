...
export default async function decorate(block) {
  ...
  const alertBar = navWrapper.querySelector('.section.alert-bar-container');
  const header = document.querySelector('.header');

  header.prepend(alertBar);
  ...
  window.addEventListener('scroll', () => {
    ...
    if (currentScroll > 70 && currentScroll > previousScroll) {
      changeBackgroundColor();
      alertBar.classList.add('hide');

    } else if (currentScroll <= 70 && currentScroll < previousScroll) {
